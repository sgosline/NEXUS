...
this.script='https://raw.githubusercontent.com/sgosline/NEXUS/master/analysis/2019-08-23/rnaSeqDiffEx.R'
...
combined.mat=round(reshape2::acast(expData,Symbol~specimenID,value.var="totalCounts",fun.aggregate=function(x) mean(x,na.rm)))
...
geneExTable=synapse$Schema(name=paste(lubridate::today(),'tumor only oneVsAll gene expression'),
    parent=tableParent,
...
pathwayTable=synapse$Schema(name=paste(lubridate::today(),'tumor only oneVsAll pathway enrichment'),
    parent=tableParent,
    cols=c( synapse$Column(name='diagnosis',columnType='STRING',maximumSize=100),
...
lapply(names(comp.list),function(comp){
    ...
    eg=entrez_list$log2FoldChange%>%set_names(entrez_list$entrezgene_id)
