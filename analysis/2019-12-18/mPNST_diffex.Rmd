---
title: "Differential expression analysis of NF1 MPNSTs"
author: "Sara Gosline"
date: "`r Sys.Date()`"
output:
  html_document:
...
tabids<-syn$tableQuery('select distinct tableId from syn21221980')$filepath%>%
  readr::read_csv()
vars="specimenID,individualID,Symbol,totalCounts,zScore,tumorType,nf1Genotype,sex,isCellLine"
...
DT::datatable(full.tab%>%group_by(tumorType)%>%summarize(`Number of Samples`=n_distinct(specimenID)))
...
```{r Calculate Diffex,warning=F, echo=F}
#calculate diffex
...
###metadata
vars<-full.tab%>%dplyr::select(specimenID,sex,tumorType)%>%
  unique()%>%
  remove_rownames()%>%
...
res.sig<-subset(res,padj<0.01)%>%subset(abs(log2FoldChange)>2)

#show table
DT::datatable(data.frame(res.sig))
```

We filter with those genes that exhibit a log2 fold change of _over_ 2 and an adjusted p.value < 0.01 to get `r nrow(res.sig)` genes.
...
full.path<-as.data.frame(kk)%>%dplyr::select(ID,Description,enrichmentScore,pvalue,p.adjust,qvalues,core_enrichment)%>%
