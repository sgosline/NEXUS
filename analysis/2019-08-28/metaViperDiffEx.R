...
syn_file='syn18455663'
expData<-syn$tableQuery(paste("SELECT * FROM",syn_file,"where isCellLine is NULL OR isCellLine='false'"))$asDataFrame()
expData$sex<-sapply(expData$sex, function (x)(gsub('female','Female',gsub('^male','Male',x))))

this.script='https://raw.githubusercontent.com/sgosline/NEXUS/master/analysis/2019-08-28/metaViperDiffEx.R'


##add in schwann cells as default
conts=which(is.na(expData$tumorType))

map<-expData%>%select(entrez,Symbol)%>%unique()
###format expression matrix
#expData<-expData%>%mutate(roundedCounts=round(totalCounts))
combined.mat=round(reshape2::acast(expData,Symbol~specimenID,value.var="viper",fun.aggregate=function(x) mean(x,na.rm=TRUE)))
missing=which(apply(combined.mat,1,function(x) any(is.na(x))))
if(length(missing)>0)
  combined.mat=combined.mat[-missing,]

###metadata
vars<-expData%>%dplyr::select(specimenID,sex,study,tumorType)%>%unique()
#vars$isCellLine=tolower(vars$isCellLine)
vars$tumorType<-make.names(vars$tumorType)
rownames(vars)<-vars$specimenID

##run DESeq2
library(DESeq2)
dds <- DESeqDataSetFromMatrix(countData = combined.mat,
  colData = select(vars,-specimenID)[colnames(combined.mat),],
...
comp.list=list(    MPNSTvsAll=c('tumorTypeMalignant.Peripheral.Nerve.Sheath.Tumor'),
...
geneExTable=synapse$Schema(name=paste(lubridate::today(),'tumor only oneVsAll metaviper expression'),
    parent=tableParent,
...
pathwayTable=synapse$Schema(name=paste(lubridate::today(),'tumor only oneVsAll metaviper pathway enrichment'),
    parent=tableParent,
    cols=c( synapse$Column(name='diagnosis',columnType='STRING',maximumSize=100),
...
lapply(names(comp.list),function(comp){
    ...
    pheatmap(log2(assay(dds)[top.30,]+1),annotation_col=df,labels_col=rep("",ncol(assay(dds))),main=paste(comp,'diff ex prots'),filename=paste(prefix,'MVtop30Heatmap.png',sep=''),cellheight=10,cellwith=10)
    #post heatmap to nexus
    syn$store(synapse$File(paste(prefix,'MVtop30Heatmap.png',sep=''),parent='syn18380760'),used=syn_file,executed=this.script)
    ...
    eg=entrez_list$log2FoldChange%>%set_names(entrez_list$entrez)
    ...
    full.path<-summary(kk)%>%dplyr::select(ID,Description,GeneRatio,pvalue,p.adjust,qvalue,geneID)%>%mutate(genelist=paste(sapply(unlist(strsplit(geneID,split='/')),function(x) entrez[match(x,entrez_list$entrez),'gene']),collapse=','))%>%dplyr::select(-geneID)
