...
##get table query
tab<-synTableQuery('SELECT * FROM syn20449214 WHERE ( ( "studyName" = \'A Nerve Sheath Tumor Bank from Patients with NF1\' OR "studyName" = \'Preclinical NF1-MPNST Platform Development\' ) )')$asDataFrame()
...
runImmuneDeconv<-function(tab,method){
  ...
  df<-dplyr::select(tab,c(studyName,sex,tumorType,specimenID,isCellLine,transplantationType))%>%
    unique()%>%
...
#store
this.script='https://raw.githubusercontent.com/sgosline/NEXUS/master/analysis/2019-09-13/pdxOnlyDeconv.R'
